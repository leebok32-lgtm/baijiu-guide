...
import { rowToAromaType } from "@/lib/supabase/mappers";
 ...
 * 명확한 에러를 던집니다 (`getActiveAromaRepository()` 가 mock으로 자동
...
export const supabaseRepository: AromaTypeRepository = {
  async getAll(): Promise<AromaType[]> {
    ...
    return (data ?? []).map(rowToAromaType);
  },
  ...
  async getBySlug(slug: string): Promise<AromaType | null> {
    const { data, error } = await client()
      ...
      .maybeSingle();
    if (error) throw error;
    return data ? rowToAromaType(data) : null;
  },
};
