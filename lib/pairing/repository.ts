...
/**
 * 페어링 데이터 접근 계층의 공통 인터페이스.
 *
 * mock 구현(`mockRepository`)과 Supabase 구현(`supabaseRepository`)은
 * 모두 이 인터페이스를 만족합니다. `lib/pairing/*` 의 public 함수들은
 * `getActivePairingRepository()` 를 통해 어느 구현을 사용할지 결정합니다.
 */
export interface PairingRepository {
  getAll(): Promise<Pairing[]>;
  getBySlug(slug: string): Promise<Pairing | null>;
}

let active: PairingRepository | null = null;

/**
 * 환경변수가 설정되어 있으면 Supabase 구현을, 그렇지 않으면 mock 구현을
 * 반환합니다. 같은 프로세스 내에서는 한 번 결정한 구현을 재사용합니다.
 */
export function getActivePairingRepository(): PairingRepository {
